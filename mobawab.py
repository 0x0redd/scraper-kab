...
class MubawabScraper:
    ...
    async def scrape_detail_page(self, listing_data: Dict) -> Optional[Dict]:
        ...
        # Get price first and skip if it's "Prix à consulter"
        price = self.get_price(soup)
        if price is None:
            logger.info(f"Skipping listing {listing_data['link']} - Price not available")
            return None
        
        # Get bathroom count and ensure it's never None
        bathrooms = self.get_bathrooms(soup)
        if bathrooms is None:
            bathrooms = 1 #todo
            logger.debug(f"Bathroom count was None, setting to 1 for property: {listing_data.get('title', 'Unknown')}")
        ...
        details = {
            **listing_data,
            ...
            "bathrooms": bathrooms,  # This will never be None
            ...
            "html_content": html  # Store HTML for debugging
        }
    ...
    def get_price(self, soup) -> Any:
        ...
        if "consult" in price_text.lower() or "Prix à consulter" in price_text:
            return None
    ...
    def get_bedrooms(self, soup) -> Optional[int]:
        ...
        bedroom_tag = soup.select_one("div.adDetailFeature span:-soup-contains('Chambres')")
        if not bedroom_tag:
            return None
            
        match = re.search(r'(\d+)', bedroom_tag.text)
        return int(match.group(1)) if match else None

    def get_bathrooms(self, soup) -> int:
        """Extract number of bathrooms"""
        try:
            # Method 1: Look for the specific structure in disFlex adDetails
            details_container = soup.select_one("div.disFlex.adDetails")
            if details_container:
                for feature_div in details_container.select("div.adDetailFeature"):
                    icon = feature_div.select_one("i.icon-bath")
                    if icon:
                        span = feature_div.select_one("span")
                        if span:
            ...
            # Method 2: Look for any div with icon-bath
            bathroom_icons = soup.select("i.icon-bath")
            for icon in bathroom_icons:
                parent_div = icon.parent
                if parent_div:
                    span = parent_div.select_one("span")
                    if span:
                        text = span.text.strip()
                        match = re.search(r'(\d+)', text)
                        if match:
            ...
            # Method 3: Look for text containing bathroom-related words
            bathroom_texts = [
                "Salles de bains",
                "Salle de bain",
                "bain",
            ...
            for text in bathroom_texts:
                ...
                for element in elements:
                    ...
                    if parent:
                        ...
                        if match:
                            return int(match.group(1))
                        elif any(word in text.lower() for word in ['bain', 'bains', 'salle']):
                            return 1

            # Method 4: Look in features list
            features = self.get_features(soup)
            for feature in features:
                if any(word in feature.lower() for word in ['bain', 'bains', 'salle']):
                    match = re.search(r'(\d+)', feature)
                    if match:
                        return int(match.group(1))
                    return 1

            logger.debug("No bathroom information found in any method, defaulting to 1")
            return 1

        except Exception as e:
            logger.error(f"Error in get_bathrooms: {str(e)}")
            return 1
    ...
    async def run_test_mode(self, num_pages: int = 2):
        """Run scraper in test mode to see scraped data"""
        try:
            await self.init_session()
            logger.info("Running in test mode...")
            
            # Process a small number of pages
            for page in range(1, num_pages + 1):
                logger.info(f"\nProcessing page {page}...")
                listings = await self.scrape_listing_page(page)
                
                for listing in listings:
                    if listing:
                        detail = await self.scrape_detail_page(listing)
                        if detail:
                            # Pretty print the scraped data
                            print("\n" + "="*50)
                            print(f"Title: {detail.get('title', 'N/A')}")
                            print(f"Price: {detail.get('price', 'N/A')}")
                            print(f"Location: {detail.get('city', 'N/A')}, {detail.get('quartier', 'N/A')}")
                            print(f"Surface: {detail.get('surface', 'N/A')} m²")
                            ...
                            print(f"Link: {detail.get('link', 'N/A')}")
                            print("="*50)
...
async def main(test_mode: bool = False, test_pages: int = 1):
    """Main async entry point"""
    scraper = MubawabScraper()
    try:
        if test_mode:
            await scraper.run_test_mode(test_pages)
        else:
            # Change max_pages to None to scrape everything, or to a number like 10
            await scraper.run(max_pages=600)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Mubawab Property Scraper')
    parser.add_argument('--test', action='store_true', help='Run in test mode')
    parser.add_argument('--test-pages', type=int, default=2, help='Number of pages to scrape in test mode')
    ...
    try:
        asyncio.run(main(test_mode=args.test, test_pages=args.test_pages))
    except KeyboardInterrupt:
        logger.info("Scraping interrupted by user")
    except Exception as e:
        logger.error(f"Scraping failed: {e}")
